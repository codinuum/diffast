...
let check_macro_body ?(name="") sp ep (tl : Token.t list) tl_obj =
  ...
  | (ASM,_,_)::_ -> [mkleaf ~pvec:[0; 0] sp ep (L.AsmDefinition "")], false

  | (IDENT x,_,_)::(IDENT _,_,_)::(COMMA,_,_)::_ when is_asm_kw x ->
  ...
  | [TYPENAME,_,ep0;IDENT x,_,_;COMMA,_,_] ->
  ...
  | [FLOAT_LITERAL _,_,_;FLOAT_LITERAL _,_,_] ->
  ...
  | [COMMA,_,_;IDENT i,s1,e1;ELLIPSIS,_,e2] -> begin
  ...
  | (IDENT _,_,_)::(EQ,_,_)::(IDENT _,_,_)::(IDENT _,_,_)::_ when begin
      try
  ...
  | (IDENT _,_,_)::(EQ,_,_)::(MINUS,_,_)::(IDENT _,_,_)::(IDENT _,_,_)::(EQ,_,_)::_ when begin
      try
  ...
  | (IDENT _,_,_)::(MINUS,_,_)::(IDENT _,_,_)::_ when begin
      try
  ...
  | (IDENT _,_,_)::(COLON,_,_)::(SLASH,_,_)::_ when begin
      try
  ...
  | (DOT,_,_)::(SLASH,_,_)::(IDENT _,_,_)::_ when begin
      true
  end -> [mkleaf sp ep (L.StringLiteral (Token.seq_to_repr tl))], false
...
| s=simple_type_specifier LPAREN el_opt=expression_list_opt RPAREN
    { 
      ...
      let lab =
        match s#label with
        | L.SimpleTypeSpecifier _ when env#scanner_keep_flag && s#nchildren = 0 -> begin
