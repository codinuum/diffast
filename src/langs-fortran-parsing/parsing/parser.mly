...
| n=PP_MACRO_NAME { let r, _ = n in mkleaf $startpos $endpos (L.Name r) }
;

stmt_end:
| EOL
    { 
(* let loc = Loc.of_lexposs $startpos $endpos in
    Printf.printf "%dL\n%!" loc.Loc.start_line *)
    }
| SEMICOLON+     { }
| SEMICOLON+ EOL { }
...
| m=pp_macro_id as_opt=ioption(pp_macro_args)
       { 
         if not (env#macro_defined m) then begin
           let body =
             Macro.mk_obj_body
               ~stat:(Macro.Resolved (Obj.repr (Tokens_.PP_MACRO_STMT m)))
               "<constrained>"
           in
...
(*   | s=pp_macro_stmt                     { s }*)
| sps=STMT                            { let _, s = sps in check_error s; s }
| sps=DO_STMT                         { let _, s = sps in check_error s; s }
| sps=FORALL_CONSTRUCT_STMT           { let _, s = sps in check_error s; s }
| sps=IF_THEN_STMT                    { let _, s = sps in check_error s; s }
| sps=SELECT_CASE_STMT                { let _, s = sps in check_error s; s }
| sps=WHERE_CONSTRUCT_STMT            { let _, s = sps in check_error s; s }
| sps=DERIVED_TYPE_STMT               { let _, s = sps in check_error s; s }

| sps=SPEC_PART_CONSTRUCT             { let _, s = sps in check_error s; s }
| sps=EXEC_PART_CONSTRUCT             { let _, s = sps in check_error s; s }
...
| sp=PROGRAM_UNIT         { at_EOPU(); let _, p = sp in check_error p; p }
...
| i=interface_body  { i }
| m=procedure_stmt  { m }
| si=INTERFACE_SPEC { let _, i = si in check_error i; i }
;
...
| c=PP_MACRO_CONST(*_CHAR*) { mkleaf $startpos $endpos (L.FormatItem (FormatItem.CharStringEditDesc c)) }
;
...
| se=EXPR              { let _, e = se in check_error e; e }
...
var_or_ref:
| d=data_ref_         { d }
| sv=VARIABLE         { let _, v = sv in check_error v; v }
...
| c=PP_MACRO_CONST(*_INT*)                 { mkleaf $startpos $endpos (L.Constant (Constant.mkppm c)) }
;
...
| sp=DERIVED_TYPE_DEF_PART    { let _, p = sp in check_error p; p }
...
| sa=ACTION_STMT             { let _, a = sa in a }
;
...
| sd=DATA_STMT_SET { let _, d = sd in check_error d; d }
;
...
| sps=SUBPROGRAM          { let _, s = sps in check_error s; s }
| p=pp_directive          { p }
| d=directive             { d }
...
| spc=CASE_BLOCK      { let _, c = spc in c }
;
